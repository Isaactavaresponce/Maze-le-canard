...
/* Ultrasónicos  */
  ...
  const int trig_df = 44;//31 48
  const int echo_df = 42;//33  46 
...
/* Variables de distancia minima a paredes */
  ...
  int LD=120;                        // Distancia delantera 
...
/*Variables de duracion de los movimientos*/
  ...
  int giros_I = 15; //16
  int giros_ISF = 15; //16
  int giros_D = 24; //20               //Tiempo de giros izquierda y derecha
...
/*  Variables de giroscopio  */
  ...
  int angulo=0;//binario si se ecuentra en alguna alteracion(si esta subiuendo una rampa o escaleras)  0=esta recto  1= esta subiendo escaleras o rampa



/* variables de sensores de piso */
  unsigned int red_1 = 10000;
  unsigned int green_1 = 10000;
  unsigned int blue_1 = 10000;

  
  unsigned int red_2 = 10000;
  unsigned int green_2 = 10000;
  unsigned int blue_2 = 10000;
...
void loop(){
  ...
  giros_D=18;   //1
  imp=25;                       //impulso 25
  LL=220;                       // Distancia lateral
  LD=85;
                          // Distancia delantera 80
      ...
      if (LFT==0){
        ...
        if (LIT ==1 && LIF==1){
         ...
         if (inercia=="IM"){
            ...
            if (LIT ==1 && LIF==1){
              izquierda_sin_fte();
              //Serial.print("izquierda_sin frente");              
           }else{
            adelante(); 
            //Serial.print("adelante  ");
           }
    ...
    if(color == "NEGRO"){
      ...
      if (inercia=="IZ"){
        ...
        if (inercia=="IZF"){
          ...
          if(victima == "detiene"){//verifica que exista victima
           detenerse();
           delay(20000);
  ...
  if(LIF==0){
    dire="adelante";
      ...
      if (MMIF<=60){//60
        ...
        // ESTA ALEJADO DE LA PRED IZQUIERDA AJUSTA A LA IZQUIERDA 
        if (MMIF >= 70){ //80
          moverse(2,1);
        }else{
          // SIGUE DE FRENTE 
...
void moverse(int vi, int vd){
  ...
  int i1 = 95; //90 se detienen los motores 
  int i2 = 95;
  int d1 = 95;
  int d2 = 95;
  ...
  if (vd==0){
      int d1 = 95; //90 se detienen los motores 
      int d2 = 95;
  }else{
    if (vd==1){
...
void Leer(){
  ...
  tcaselect(1);
    ...
    if ((ang_x>= 86 )or(ang_y>= -1 and ang_y<= 1 )){
      angulo=0;
    }
    else{
      angulo=1;
     ...
     //si se encuentra en algun angulo diferente que no sea el del piso no va a detectar
    if(angulo==0){
   ...
   // else{
    ...
    if(((red_1<1000) and (green_1<1000) and (blue_1<1000)) or 
       ((red_2<1000) and (green_2<100) and (blue_2<1000)))
       
       {
       color = "NEGRO";
    }else{
      if(((red_1<600) and (green_1<1000) and (blue_1<1600)) or 
       ((red_2<600) and (green_2<1000) and (blue_2<1600))){
       color = "AZUL";
      }
      else{
        color="BLANCO";
